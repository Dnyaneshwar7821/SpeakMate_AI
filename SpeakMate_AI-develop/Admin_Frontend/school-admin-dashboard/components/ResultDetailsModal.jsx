...
export function ResultDetailsModal({ isOpen, result, onClose }) {
    ...
    const isPassed = isResultPassed(result.status);
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} maxWidth="max-w-2xl" title="Test Result Breakdown">
            <div className="mt-4 flex flex-col gap-6">
                ...
                {/* Header Summary */}
                <div className={`flex flex-col gap-4 rounded-2xl border p-5 sm:flex-row sm:items-center sm:justify-between ${isPassed ? 'border-emerald-500/20 bg-emerald-500/5' : 'border-rose-500/20 bg-rose-500/5'
                    }`}>
                    <div>
                        <div className="flex items-center gap-3">
                            <h3 className="text-xl font-bold text-[var(--text-primary)]">{result.studentName}</h3>
                            <span className={`inline-flex items-center gap-1 rounded-full px-2.5 py-1 text-xs font-bold ${isPassed ? 'bg-emerald-500/10 text-emerald-500' : 'bg-rose-500/10 text-rose-500'
                                }`}>
