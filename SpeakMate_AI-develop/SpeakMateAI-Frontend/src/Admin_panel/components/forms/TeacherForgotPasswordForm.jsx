...
export function TeacherForgotPasswordForm({
  ...
  const handleSubmit = async (event) => {
    ...
    if (result.success) {
      const targetRoute = otpRoute || ROUTES.TEACHER_VERIFY_OTP;
      navigate(`${targetRoute}?email=${encodeURIComponent(cleanEmail)}`, {
  ...
  return (
    <form className="mt-6 space-y-4" onSubmit={handleSubmit} noValidate>
      <AdminAlert tone="info">
        Enter your registered email address and we will send a 6-digit OTP code to verify your account.
      </AdminAlert>

      {error && <AdminAlert tone="error">{error}</AdminAlert>}

      <AdminInput
        id="teacher-forgot-email"
        name="email"
        label="Email address"
        type="email"
