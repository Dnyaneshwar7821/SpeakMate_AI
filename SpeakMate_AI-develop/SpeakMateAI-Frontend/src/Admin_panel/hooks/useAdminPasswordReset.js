...
import {
  ...
  validateAdminResetWithTempPasswordForm,
...
export function useAdminPasswordReset() {
  ...
  const resetWithTemporaryPassword = useCallback(
    async ({ email, temporaryPassword, newPassword, confirmPassword, role }) => {
      const validationErrors = validateAdminResetWithTempPasswordForm({
        email,
        temporaryPassword,
        newPassword,
        confirmPassword,
      ...
      return runRequest(
        () => adminAuthService.resetPasswordWithTemporary({ email, temporaryPassword, newPassword, role }),
        "Unable to set the new password. Please verify your temporary password."
      );
    },
    [runRequest]
  );

  return { requestOtp, verifyOtp, resetPassword, resetWithTemporaryPassword, isLoading, error, fieldErrors, clearErrors };
}
