...
export class ModelLoader {
  ...
  static async loadModel(modelPath, pixiApp) {
    ...
    try {
      ...
      const scaleMultiplier = isWanko ? 1.35 : isShizuku ? 1.18 : (isFullBody ? 2.30 : 1.05);
      const scale = (screenHeight * scaleMultiplier) / nativeHeight;
      model.scale.set(scale, scale);

      model.x = screenWidth / 2;
      model.y = Math.max(isWanko ? 8 : isShizuku ? 4 : (isFullBody ? 6 : 16), screenHeight * (isWanko ? 0.08 : isShizuku ? 0.02 : (isFullBody ? 0.04 : 0.10)));
