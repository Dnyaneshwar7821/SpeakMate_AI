...
export function AdminResetPasswordForm({
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(
      () =>
        navigate(
          `${loginRoute}${email ? `?email=${encodeURIComponent(email)}` : ""}`,
          { replace: true, state: { email, resetSuccess: true } }
        ),
      SUCCESS_REDIRECT_DELAY_MS
    );
    return () => window.clearTimeout(timer);
  }, [isSuccess, loginRoute, navigate, email]);
  ...
  const handleSubmit = async (event) => {
    ...
    if (result.success) {
      if (email) {
        try {
          localStorage.setItem(`pwd_set_${email.toLowerCase().trim()}`, "true");
        } catch (_) {}
      }
      setIsSuccess(true);
    }
