 ...
 *  - Spacing density based on user configuration
 */
        ...
        >
            {({ isActive }) => (
                <>
                    ...
                    {item.badge && (
                        <span className="ml-auto rounded-full bg-[var(--color-accent)] px-1.5 py-0.5 text-[10px] font-bold text-white shadow-xs">
                            {item.badge}
                        </span>
                    )}
                </>
                ...
                <div className="no-scrollbar flex-1 overflow-y-auto">
                    <nav className="flex flex-col gap-0.5 px-3 py-3">
                        {ADMIN_SIDEBAR_MENU.map((item) => {
                            const itemWithBadge = item.id === "notifications" && unreadCount > 0 ? { ...item, badge: formattedUnreadCount } : item;
                            return <SidebarLink key={item.id} item={itemWithBadge} />;
                        })}
                        ...
                        <button
                            ...
                            className={[
                                "group relative flex w-full items-center gap-3 rounded-xl px-3 text-[13px] font-medium text-rose-500 transition-all duration-200 hover:bg-rose-500/10",
                                sidebarDensity === "compact" ? "py-1.5" : "py-2.5"
                            ].join(" ")}
                        >
