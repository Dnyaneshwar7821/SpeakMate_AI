...
import { adminDashboardApi } from "../../src/services/adminDashboardApi";
...
const mapBackendStudent = (s) => {
  ...
  return {
    ...
    progress: {
      completedLessons: s.totalLessonsCompleted || 0,
      totalLessons: 50,
      percentage: s.totalLessonsCompleted ? Math.min(100, Math.round((s.totalLessonsCompleted / 50) * 100)) : 0,
      quizzes: s.totalGrammarSessions || 0,
      averageScore: s.totalLessonsCompleted > 0 ? Math.min(100, 75 + s.totalLessonsCompleted) : 0,
      lastActive: s.createdAt ? s.createdAt.slice(0, 10) : new Date().toISOString().slice(0, 10),
      history: generateHistory(s.totalLessonsCompleted > 0 ? Math.min(100, 75 + s.totalLessonsCompleted) : 75)
    },
    raw: s
  };
};
