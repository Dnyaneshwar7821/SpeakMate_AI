...
public class AnswerSynthesizer {
	...
	private String intentGuidance(AssistantIntent intent) {
		...
		return switch (intent) {
			...
			case STUDENT_PERFORMANCE -> "Answer ONLY the metric(s) the question actually asked about for this student — do NOT list every metric when the question targets one. Map the question to the matching fields and report just those: lessons -> lessonsCompleted (plus lessonsStarted/lessonsPending when helpful); XP/points -> xp (plus level); streak -> currentStreak/longestStreak; practice time -> totalPracticeMinutes; speaking -> totalSpeakingSessions; grammar -> totalGrammarChecks; vocabulary -> totalVocabularyWords. Open with the student's name, standard and division once for context. Give a full multi-metric summary ONLY when the question is a broad 'how is this student doing' question. A count of 0 is a valid number, so state it explicitly rather than saying the data is unavailable. Be encouraging and keep it to a couple of lines. If the data describes a person who is NOT a student (it carries a personRole field or a summary stating the person is not a student), still answer helpfully: state that they are not a student so no XP/learning metric exists for them, and report their role EXACTLY as given in personRole — never substitute, upgrade, or invent a different role (for example, never call a 'User' a 'School Admin').";
