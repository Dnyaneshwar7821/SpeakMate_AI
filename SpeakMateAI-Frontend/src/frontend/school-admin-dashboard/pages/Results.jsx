...
import { isResultPassed } from "@school-admin/utils/resultStatus";
...
export function Results() {
    const { results, searchTerm, setSearchTerm, isLoading, error } = useResults();
    ...
    const avgScore = totalCount > 0
        ...
        : 0;
    const totalPassed = filteredResults.filter((r) => isResultPassed(r.status)).length;
    const totalFailed = filteredResults.filter((r) => !isResultPassed(r.status)).length;
    ...
    return (
        <div className="space-y-5 sm:space-y-6">
            ...
            >
                ...
                <Button onClick={handleExport} variant="secondary" disabled={!filteredResults.length} className="!h-10 w-full shrink-0 sm:w-auto">
            ...
            {isLoading && (
                <div className="rounded-xl border border-[var(--border-subtle)] bg-[var(--bg-surface)] p-8 text-center text-sm text-[var(--text-secondary)]">
                    Loading results…
                </div>
            )}
            {error && !isLoading && (
                <div role="alert" className="rounded-xl border border-rose-500/20 bg-rose-500/5 p-5 text-sm text-rose-500">
                    {error}
                </div>
            )}

            {/* Table Section */}
            {!isLoading && !error && <SectionCard
                title="All Results"
