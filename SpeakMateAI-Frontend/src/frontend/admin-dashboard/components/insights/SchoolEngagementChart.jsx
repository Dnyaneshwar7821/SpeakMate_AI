...
/**
 * Custom Tooltip for School Engagement Comparison
 */
function SchoolTooltip({ active, payload, label }) {
...
/**
 * frontend/admin-dashboard/components/insights/SchoolEngagementChart.jsx
 *
 * Primary Super Admin comparison chart.
 * Compares schools by Total, Active, and Inactive student counts.
 */
export function SchoolEngagementChart({ data = [], selectedSchool = "All Schools" }) {
  const hasData = Array.isArray(data) && data.length > 0;

  return (
    <SectionCard
      title="School Engagement Comparison"
      subtitle={
        selectedSchool !== "All Schools"
          ? `Showing student activity for ${selectedSchool}`
          : "Comparing student activity across registered schools"
      }
      action={
        <div className="flex items-center gap-2 text-xs font-semibold text-[var(--text-muted)]">
          <School className="h-4 w-4 text-[var(--color-primary)]" />
          <span>{hasData ? `${data.length} School${data.length === 1 ? "" : "s"}` : "0 Schools"}</span>
        </div>
      }
    >
        ...
        {!hasData ? (
          <div className="flex h-full w-full flex-col items-center justify-center p-6 text-center">
            ...
            <p className="mt-1 text-xs text-[var(--text-secondary)]">
              Try adjusting your filter selection to view cross-school data.
            </p>
          </div>
        ) : (
...
export default SchoolEngagementChart;
