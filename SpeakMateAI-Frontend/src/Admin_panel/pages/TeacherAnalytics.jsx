...
function TopPerformers({ students = [] }) {
    const list = Array.isArray(students) ? students.filter((s) => (s.score ?? s.overallScore ?? 0) >= 60) : [];
...
function StudentAnalyticsModal({ isOpen, onClose, student, detail, loadingDetail }) {
    ...
    const rawStatus = student.status || detail?.profile?.status;
    const status = (rawStatus === "ACTIVE" || rawStatus === "Active" || student.active === true) ? "ACTIVE" : "INACTIVE";
