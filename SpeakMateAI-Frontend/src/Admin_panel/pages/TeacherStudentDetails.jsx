...
const statusStyles = {
    ...
    Average: "bg-violet-50 text-violet-700 ring-violet-600/10 dark:bg-violet-500/15 dark:text-violet-400 dark:ring-violet-500/20",
...
const activityDotStyles = {
    indigo: "bg-indigo-500 ring-indigo-100 dark:ring-indigo-900/50",
    emerald: "bg-emerald-500 ring-emerald-100 dark:ring-emerald-900/50",
    amber: "bg-amber-500 ring-amber-100 dark:ring-amber-900/50",
    violet: "bg-violet-500 ring-violet-100 dark:ring-violet-900/50",
};

const achievementStyles = {
    indigo: "bg-indigo-50 text-indigo-700 ring-indigo-100 dark:bg-indigo-500/15 dark:text-indigo-400 dark:ring-indigo-500/20",
    amber: "bg-amber-50 text-amber-700 ring-amber-100 dark:bg-amber-500/15 dark:text-amber-400 dark:ring-amber-500/20",
    emerald: "bg-emerald-50 text-emerald-700 ring-emerald-100 dark:bg-emerald-500/15 dark:text-emerald-400 dark:ring-emerald-500/20",
};
                ...
                <div className="flex flex-col gap-6 p-5 sm:p-6 lg:flex-row lg:items-center lg:justify-between">
                    <div className="flex min-w-0 items-center gap-4">
                        ...
                        <div className="min-w-0">
                            <h1 className="text-2xl font-black text-slate-950 dark:text-slate-50 sm:text-3xl">{student.name}</h1>
                            <div className="mt-2 flex flex-wrap items-center gap-x-4 gap-y-2 text-sm font-medium text-slate-500 dark:text-slate-400">
                    ...
                    <div className="grid grid-cols-2 gap-3 sm:flex sm:items-center">
                        <div className="rounded-xl border border-slate-200 bg-white px-4 py-3 dark:border-slate-800 dark:bg-slate-900">
                            ...
                            <p className="mt-1 text-xl font-black text-slate-950 dark:text-slate-100">{student.overallProgress}%</p>
                        </div>
                        <div className="rounded-xl border border-slate-200 bg-white px-4 py-3 dark:border-slate-800 dark:bg-slate-900">
...
function PerformanceSummary({ student }) {
    const grammarVal = student.grammar ?? student.grammarScore ?? 0;
    const vocabularyVal = student.vocabulary ?? student.vocabularyScore ?? 0;
    const speakingVal = student.speaking ?? student.speakingScore ?? 0;
    const listeningVal = student.listening ?? student.listeningScore ?? 0;
    const completionVal = student.practiceCompletion ?? student.practice?.weeklyCompletion ?? 85;

    const metrics = [
        ["Overall Progress", student.overallProgress ?? 0, progressStyles[student.status]],
        ["Grammar", grammarVal, "bg-indigo-500"],
        ["Vocabulary", vocabularyVal, "bg-amber-500"],
        ["Speaking", speakingVal, "bg-emerald-500"],
        ["Listening", listeningVal, "bg-violet-500"],
        ["Practice Completion", completionVal, "bg-rose-500"],
    ];
                    ...
                    <ul className="mt-5 space-y-3">
                        {group.items.map((item, index) => {
                            if (typeof item === "object" && item !== null) {
                                const key = item.id || `${item.skill || "skill"}-${item.score || index}-${index}`;
                                return (
                                    <li key={key} className="flex items-center justify-between gap-3 text-sm leading-6 text-slate-600 dark:text-slate-300">
                                        <div className="flex items-center gap-3">
                                            <span className={`h-1.5 w-1.5 shrink-0 rounded-full ${group.tone === "emerald" ? "bg-emerald-500" : "bg-amber-500"}`} />
                                            <span className="font-semibold text-slate-800 dark:text-slate-200">{item.skill}</span>
                                        </div>
                                        <div className="flex items-center gap-2 text-xs font-bold">
                                            {item.score != null && (
                                                <span className="text-slate-700 dark:text-slate-300">{Math.round(item.score)}%</span>
                                            )}
                                            {item.label && (
                            ...
                            const stringKey = typeof item === "string" ? `${item}-${index}` : `item-${index}`;
                            return (
                                <li key={stringKey} className="flex gap-3 text-sm leading-6 text-slate-600 dark:text-slate-300">
...
function PracticeOverview({ practice = {} }) {
    const practiceDaysVal = practice.practiceDays != null ? `${practice.practiceDays} days` : "0 days";
    const currentStreakVal = practice.currentStreak != null ? `${practice.currentStreak} days` : "0 days";
    const lastPracticeVal = practice.lastPractice || "No activity";
    const completionVal = Math.round(practice.weeklyCompletion ?? 0);

    const items = [
        ["Practice Days", practiceDaysVal],
        ["Current Streak", currentStreakVal],
        ["Last Practice", lastPracticeVal],
        ["Weekly Completion", `${completionVal}%`],
    ];
        ...
        <Card className="p-5 sm:p-6">
            ...
            <dl className="mt-5 grid grid-cols-2 gap-3">
                {items.map(([label, value]) => (
                    <div key={label} className="rounded-xl bg-slate-50 p-4 ring-1 ring-slate-100 dark:bg-slate-900 dark:ring-slate-800">
                        <dt className="text-xs font-bold uppercase tracking-wide text-slate-400">{label}</dt>
                        <dd className="mt-2 text-base font-black text-slate-900 dark:text-slate-100">{value}</dd>
                    </div>
                ))}
            </dl>
            <div className="mt-4">
                <ProgressBar value={completionVal} tone="bg-emerald-500" />
            </div>
        </Card>
    );
}
...
export function TeacherStudentDetails() {
    ...
    useEffect(() => {
        const fetchStudentDetail = async () => {
            setLoading(true);
            try {
                // Parse numeric ID if string (e.g. "1" or "stu-1")
                const numericId = studentId?.replace(/\D/g, "") || "1";
                const res = await teacherDataApi.getStudentDetail(numericId);
                if (res) {
                    const prof = res.profile || {};
                    const perf = res.performance || {};
                    const stats = res.practiceStatistics || {};

                    const speakingVal = perf.speakingScore ?? perf.speaking ?? null;
                    const grammarVal = perf.grammarScore ?? perf.grammar ?? null;
                    const vocabularyVal = perf.vocabularyScore ?? perf.vocabulary ?? null;
                    const listeningVal = perf.listeningScore ?? perf.listening ?? null;

                    // Filter only attempted scores (> 0) to avoid diluting average with unattempted 0s
                    const activeScores = [speakingVal, grammarVal, vocabularyVal, listeningVal].filter((s) => s !== null && s !== undefined && !isNaN(s) && Number(s) > 0);
                    const computedAvg = activeScores.length > 0 ? Math.round(activeScores.reduce((a, b) => a + Number(b), 0) / activeScores.length) : 0;
                    const calculatedOverall = (perf.overallScore && perf.overallScore > 0) ? perf.overallScore : (activeScores.length > 0 ? computedAvg : (prof.overallProgress ?? 0));

                    const mappedStudent = {
                        id: prof.id || studentId,
                        ...
                        overallProgress: Math.round(calculatedOverall),
                        lastActive: prof.lastActive || "Recently",
                        speakingScore: speakingVal !== null ? Math.round(speakingVal) : 0,
                        grammarScore: grammarVal !== null ? Math.round(grammarVal) : 0,
                        vocabularyScore: vocabularyVal !== null ? Math.round(vocabularyVal) : 0,
                        listeningScore: listeningVal !== null ? Math.round(listeningVal) : 0,
                        speaking: speakingVal !== null ? Math.round(speakingVal) : 0,
                        grammar: grammarVal !== null ? Math.round(grammarVal) : 0,
                        vocabulary: vocabularyVal !== null ? Math.round(vocabularyVal) : 0,
                        listening: listeningVal !== null ? Math.round(listeningVal) : 0,
                        practiceCompletion: Math.round(perf.practiceCompletion ?? stats.weeklyCompletion ?? 0),
                        recentActivity: res.recentActivity || [],
                        strengths: res.strengths || [],
                        improvementAreas: res.improvementAreas || [],
                        achievements: res.achievements || [],
                        practice: {
                            currentStreak: res.currentStreak ?? stats.currentStreak ?? 0,
                            practiceMinutes: stats.totalPracticeMinutes ?? 0,
                            practiceDays: res.practiceStatistics?.totalPracticeDays ?? (res.recentActivity ? new Set(res.recentActivity.map(a => a.time ? String(a.time).substring(0, 10) : "")).size : 0),
                            lastPractice: res.lastPracticeDate ? new Date(res.lastPracticeDate).toLocaleDateString() : (res.recentActivity && res.recentActivity.length > 0 ? "Recently" : "No activity"),
                            weeklyCompletion: res.attendanceRate ? Math.round(res.attendanceRate) : (stats.weeklyCompletion ?? 0)
                        }
                    };
                    setStudentData(mappedStudent);
                }
            } catch (err) {
                console.warn("Notice: Student detail API fetch notice:", err);
            } finally {
                setLoading(false);
            }
        };
