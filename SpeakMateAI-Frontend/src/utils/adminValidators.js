 ...
 * Password, and OTP Verification forms.
 */

import { isAdminRole } from "@/Admin_panel/constants/adminRoles";
...
export function validateAdminResetWithTempPasswordForm({ email, temporaryPassword, newPassword, confirmPassword, role }) {
  const errors = {};
  addRoleError(errors, role);
  ...
  const requiredError = getAdminPasswordError(newPassword);
  if (requiredError) {
    errors.newPassword = requiredError;
  } else {
    const strengthError = getAdminPasswordStrengthError(newPassword);
    if (strengthError) {
      errors.newPassword = strengthError;
    }
  }

  if (temporaryPassword && newPassword && temporaryPassword.trim() === newPassword) {
    errors.newPassword = "New password must be different from your temporary password.";
  }
