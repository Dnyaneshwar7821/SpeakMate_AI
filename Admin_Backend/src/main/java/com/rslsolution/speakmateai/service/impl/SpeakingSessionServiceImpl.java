...
public class SpeakingSessionServiceImpl implements SpeakingSessionService {
	...
	public SpeakingEndResponse endSession(Long id) {
		...
		try {
			...
			Long studentId = session.getStudent() != null ? session.getStudent().getId() : null;
			if (session.getStudent() != null && session.getStudent().getEmail() != null) {
				notificationService.sendNotification(session.getStudent().getEmail(),
						"Speaking Session Complete! 🎙️",
						"Great job! You practiced \"" + session.getScenario() + "\" for " + sessionMinutes + " min and earned " + xp + " XP.",
						NotificationType.SPEAKING_ACTIVITY_COMPLETED,
						studentId,
						"STUDENT");
			}
			if (studentId != null) {
				String studentName = (session.getStudent().getFirstName() + " " + (session.getStudent().getLastName() != null ? session.getStudent().getLastName() : "")).trim();
				notificationService.notifyAdmins("Speaking Activity Completed", "Student " + studentName + " completed a speaking session on \"" + session.getScenario() + "\".", NotificationType.SPEAKING_ACTIVITY_COMPLETED, studentId, "STUDENT");
			}
