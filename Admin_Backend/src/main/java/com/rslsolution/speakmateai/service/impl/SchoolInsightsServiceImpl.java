...
public class SchoolInsightsServiceImpl implements SchoolInsightsService {
    ...
    private List<Speaker> buildTopSpeakers(List<SpeakingSession> sessions) {
        Map<Long, List<SpeakingSession>> byStudent = sessions.stream()
                .filter(session -> session.getStudent() != null && session.getStudent().getId() != null)
                .collect(Collectors.groupingBy(session -> session.getStudent().getId()));
        return byStudent.values().stream()
                .map(studentSessions -> {
                    Student student = studentSessions.get(0).getStudent();
                    return Speaker.builder()
                            .id(student.getId())
                            .name((student.getFirstName() + " " + student.getLastName()).trim())
                            .standard(student.getStandard())
                            .score(round(averageScores(studentSessions, SpeakingSession::getPronunciationScore)))
                            .build();
                })
